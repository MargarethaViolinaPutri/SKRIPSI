...
export default function TestForm({ test, courses }: Props) {
    ...
    const onSubmit = async (e: FormEvent) => {
        e.preventDefault();
        if (test) {
            await put(route('master.test.update', { id: test.id }));
        } else {
            await post(route('master.test.store'));
        }
    };
            ...
            <div className="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => setData('duration_in_minutes', parseInt(e.target.value) || undefined)}
                    />
                </div>
                <div>
